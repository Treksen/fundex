...
export default function GovernancePage() {
  ...
  useEffect(() => {
    const ch = supabase
      .channel("governance-rt")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "action_approvals" },
        fetchData,
      )
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "action_approval_votes" },
        fetchData,
      )
      .subscribe();
  ...
  const handleVote = async (approvalId, vote) => {
    ...
    if (error) toast.error(error.message);
      ...
      toast.success("✅ Action approved by quorum!");
    else if (data?.status === "rejected")
      toast.error("❌ Action rejected by quorum.");
    else
      toast.success(
        `Vote recorded — ${data?.approved || 0}/${data?.needed || 2} approvals`,
      );
  ...
  const myVote = (approvalId) =>
    votes[approvalId]?.find((v) => v.voter_id === profile?.id);
  const canVote = (appr) =>
    appr.status === "pending" &&
  ...
  return (
    <div>
      <PageHeader
        title="Governance"
        subtitle="Multi-signature approval workflows — 2-of-3 required"
        onRefresh={fetchData}
        loading={loading}
      >
        {isAdmin && (
      ...
      {pendingCount > 0 && (
        ...
        >
          ...
          >
            {pendingCount} proposal{pendingCount !== 1 ? "s" : ""} awaiting your
            vote
          </span>
        </div>
      )}
        ...
        {["", "pending", "approved", "rejected", "cancelled"].map((s) => (
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              background:
                filterStatus === s ? "var(--olive)" : "var(--bg-elevated)",
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {filtered.map((appr) => {
            const cfg = ACTION_LABELS[appr.action_type] || {
              label: appr.action_type,
              icon: "📋",
            ...
            const pct =
              appr.min_approvals > 0
                ? Math.round((appr.approved_count / appr.min_approvals) * 100)
                : 0;
            ...
            return (
              <div
                ...
                style={{
                  ...
                  opacity: deletingId === appr.id ? 0.5 : 1,
                }}
              >
                <div
                  style={{
                ...
                >
                  ...
                  >
                    <MemberAvatar
                      name={appr.profiles?.name}
                      avatarUrl={appr.profiles?.avatar_url}
                      size={28}
                      fontSize={11}
                    />
                    <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
                      {appr.profiles?.name?.split(" ")[0]}
                    </span>
                    {isAdmin && (
                      <AdminActions
                    ...
                    >
                      <div
                        style={{
                          ...
                          background:
                            appr.rejected_count >= appr.min_approvals
                              ? "var(--accent-red)"
                              : "var(--olive)",
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    ...
                    {isAdmin && appr.status === "pending" && (
                      <button
                        ...
                        onClick={() => handleCancelApproval(appr.id)}
                      >
                        Cancel
                      </button>
                    )}
                ...
                {mine && (
                  ...
                  >
                    ...
                    You {mine.vote === "approve" ? "approved" : "rejected"} this
                    proposal
                  </div>
                )}
              </div>
      ...
      <div className="modal" style={{ maxWidth: 500 }}>
        ...
        <div className="modal-body">
          {!isEdit && (
            ...
            >
              🗳️ Requires <strong>2 approvals out of 3 members</strong> to
              proceed.
            </div>
          )}
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, action_type: e.target.value }))
                }
              >
            ...
            <div className="form-group">
              ...
              <textarea
                ...
                value={form.metadata}
                onChange={(e) =>
                  setForm((p) => ({ ...p, metadata: e.target.value }))
                }
                rows={3}
              />
            </div>
            <div className="flex gap-3">
              ...
              >
                {loading ? (
                  ...
                  <>
                    <Save size={14} /> {isEdit ? "Save Changes" : "Submit"}
                  </>
                )}
              </button>
            </div>
