...
export default function CashFlowPage() {
  ...
  const riskFlag = proj30
    ? (Number(proj30.projected_balance) < 0 ? 'critical'
       : Number(proj30.projected_balance) < currentBalance * 0.3 ? 'warning' : 'safe')
    : 'safe'

  const cashFlowChartData = useMemo(() => ({
    labels: monthly.map(m => m.label),
    datasets: [
      { label: 'Inflows', data: monthly.map(m => m.in), backgroundColor: 'rgba(13,156,94,0.7)', borderRadius: 4 },
      { label: 'Outflows', data: monthly.map(m => m.out), backgroundColor: 'rgba(220,53,69,0.7)', borderRadius: 4 },
    ]
  }), [monthly])
  ...
  const balanceChartData = useMemo(() => ({
    ...
    datasets: [{
      ...
      borderColor: riskFlag === 'critical' ? 'var(--accent-red)' : 'var(--olive)',
      backgroundColor: riskFlag === 'critical' ? 'rgba(220,53,69,0.10)' : 'rgba(90,138,30,0.10)',
  ...
  const chartOpts = {
    ...
    plugins: { legend: { labels: { color: '#8a9a90', font: { size: 11 } } }, tooltip: { callbacks: { label: ctx => ` KES ${ctx.raw.toLocaleString()}` } } },
    scales: {
      x: { grid: { display: false }, ticks: { color: '#8a9a90', font: { size: 10 } } },
      y: { grid: { color: 'rgba(128,128,128,0.1)' }, ticks: { color: '#8a9a90', font: { size: 10 }, callback: v => `${(v/1000).toFixed(0)}k` } }
    }
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        loading={loading}
      >
        <button className="btn btn-primary" onClick={runForecast} disabled={generating}>
          {generating ? <><div className="spinner" style={{width:14,height:14}}/> Forecasting…</> : <><Zap size={14}/> Run Forecast</>}
        </button>
      </PageHeader>
      ...
      {riskFlag !== 'safe' && (
        <div style={{
          background: riskFlag==='critical' ? 'rgba(220,53,69,0.08)' : 'rgba(230,144,10,0.08)',
          border: `1px solid ${riskFlag==='critical' ? 'rgba(220,53,69,0.25)' : 'rgba(230,144,10,0.25)'}`,
          borderRadius: 10, padding: '12px 16px', marginBottom: 16,
          display: 'flex', alignItems: 'center', gap: 10,
        }}>
          <AlertTriangle size={16} style={{color: riskFlag==='critical' ? 'var(--accent-red)' : 'var(--accent-amber)', flexShrink:0}}/>
          <span style={{fontSize:14,fontWeight:600,color: riskFlag==='critical'?'var(--accent-red)':'var(--accent-amber)'}}>
            {riskFlag==='critical'
      ...
      <div className="page-tab-bar">
        {[['overview','Overview'],['reserve','🛡️ Reserve Fund'],['virtual','Virtual Accounts'],['capital','Capital Accounts']].map(([id,label]) => (
      ...
      {activeTab === 'overview' && (
        <div style={{display:'flex',flexDirection:'column',gap:16}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:10}}>
            {[
              {label:'Current Balance',value:formatCurrency(currentBalance),color:'var(--navy-light)',sub:'Live pool'},
              {label:'Liquid Available',value:formatCurrency(liquidBalance),color:'var(--accent-emerald)',sub:`${liquidPct}% of pool`},
              {label:'In 30 Days',value:proj30?formatCurrency(proj30.projected_balance):'Run forecast',color:riskFlag==='safe'?'var(--olive)':riskFlag==='warning'?'var(--accent-amber)':'var(--accent-red)',sub:'Projection'},
              {label:'In 60 Days',value:proj60?formatCurrency(proj60.projected_balance):'—',color:'var(--text-secondary)',sub:'Projection'},
              {label:'In 90 Days',value:proj90?formatCurrency(proj90.projected_balance):'—',color:'var(--text-muted)',sub:'Projection'},
            ].map(s => (
              <div key={s.label} className="card" style={{padding:'12px 14px'}}>
                <div style={{fontSize:11,color:'var(--text-muted)',fontWeight:700,textTransform:'uppercase',marginBottom:4}}>{s.label}</div>
          ...
          <div className="card">
            <div className="card-header"><span className="card-title">Balance Timeline + 90-Day Forecast</span></div>
            <div style={{height:240}}>
              {monthly.length > 0 ? <Line data={balanceChartData} options={chartOpts}/> :
          ...
          {/* Cash flow bar chart */}
          <div className="card">
            <div className="card-header"><span className="card-title">Monthly Cash Flow (Inflows vs Outflows)</span></div>
            <div style={{height:220}}>
              {monthly.length > 0 ? <Bar data={cashFlowChartData} options={chartOpts}/> :
                  ...
                  <tbody>
                    {monthly.map(m => (
                      <tr key={m.label}>
                        ...
                        <td style={{textAlign:'right',fontFamily:'var(--font-mono)',color:'var(--accent-emerald)',fontSize:13}}>{formatCurrency(m.in)}</td>
      ...
      {activeTab === 'reserve' && (
        <ReserveFundPanel onUpdate={fetchData} />
      )}

      {activeTab === 'virtual' && (
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          <div style={{background:'rgba(26,36,114,0.06)',border:'1px solid rgba(26,36,114,0.15)',borderRadius:8,padding:'10px 14px',fontSize:13,color:'var(--text-secondary)'}}>
      ...
      {activeTab === 'capital' && (
        <div className="card" style={{padding:0,overflow:'hidden'}}>
          <div style={{padding:'14px 16px 10px',borderBottom:'1px solid var(--border)'}}>
            ...
            <p style={{fontSize:12,color:'var(--text-muted)',marginTop:2}}>Paid-in capital, retained earnings, and profit allocations per member</p>
          </div>
          {capital.length === 0 ? (
            <div className="empty-state"><p>No capital accounts yet — run the migration and sync</p></div>
          ) : (
            <div className="table-container" style={{border:'none',borderRadius:0}}>
              <table>
                <thead>
                ...
                <tbody>
                  {capital.map(c => (
                    <tr key={c.id}>
                      ...
                      <td style={{textAlign:'right',fontFamily:'var(--font-mono)',fontSize:13}}>{formatCurrency(c.paid_in_capital)}</td>
                      <td style={{textAlign:'right',fontFamily:'var(--font-mono)',fontSize:13,color:'var(--accent-emerald)'}}>{formatCurrency(c.retained_earnings)}</td>
