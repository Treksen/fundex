...
export default function AddTransactionModal({ onClose, onSuccess, preselectedUser }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const txData = {
      ...
      // Deposits now start as 'pending_verification' — an admin
      // (or, if the depositor is an admin, another member) must
      // confirm the funds arrived in the bank and supply a bank
      // verification code via the verify_deposit RPC before the
      // deposit counts toward equity/contributions.
      // Withdrawals continue with their existing approval flow.
      // Adjustments remain completed (admin-only by design).
      status: form.type === 'withdrawal'
                ? 'pending'
                : form.type === 'deposit'
    ...
    if (form.type === 'deposit') {
      toast.success('Deposit recorded — awaiting admin verification 🕓')
    } else {
      ...
      <div className="modal">
        ...
        <div className="modal-body">
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              ...
              {form.type === 'deposit' && (
                <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>
                  This is the reference YOU received when sending the money. An
                  admin will later add a separate bank-side code to verify the deposit.
                </p>
              )}
            </div>
            ...
            {form.type === 'deposit' && (
              <div style={{ background: 'rgba(58,77,181,0.06)', border: '1px solid rgba(58,77,181,0.22)', borderRadius: 8, padding: 12, marginBottom: 16 }}>
                <p style={{ fontSize: 13, color: 'var(--accent-blue, #3a4db5)', fontWeight: 600, marginBottom: 4 }}>
                  🕓 Deposit will await verification
                </p>
                <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                  After you record this deposit, an admin will confirm the funds
                  in the group bank account and stamp it with a bank verification
                  code. Only then does it count toward your equity and the
                  group pool.
                </p>
              </div>
            )}
